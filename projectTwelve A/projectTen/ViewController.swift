...
class ViewController: UICollectionViewController, UIImagePickerControllerDelegate, UINavigationControllerDelegate {
    ...
    override func viewDidLoad() {
        ...
        // we are getting the data from disk
        if let savedPeople = defaults.object(forKey: "people") as? Data {
            // here we are deconding the data and we know it the type of person array
            if let decodePeople = try? NSKeyedUnarchiver.unarchiveTopLevelObjectWithData(savedPeople) as? [Person] {
                // succeded of retriving data and putting it back into people variable
                // which is a person array
                // don't need to reload collectionview this code will happen before it loads
                // then itemsinsec will be excuted with cell for item at
                people = decodePeople
            }
        }
